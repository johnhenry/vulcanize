...
function handleMainDocument() {
  ...
  if (options.csp) {
    ...
    $(constants.JS_INLINE).each(function(index) {
      var el = $(this);
      var content = getTextContent(el);
      var scriptName = path.basename(options.output, (index ? ("." + index) : "") + '.html') + '.js';
      fs.writeFileSync(path.resolve(options.outputDir, scriptName), content, 'utf8');
